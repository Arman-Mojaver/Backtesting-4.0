...
def test_from_ids(session, points):
    ...
    assert lists_are_equal(
        MoneyManagementStrategy.query.from_ids(ids=[point_1.id]), [point_1]
    )
    assert lists_are_equal(
        MoneyManagementStrategy.query.from_ids(ids=[point_2.id]), [point_2]
    )
    assert lists_are_equal(
        MoneyManagementStrategy.query.from_ids(ids=[point_1.id, point_2.id]), points
    )
