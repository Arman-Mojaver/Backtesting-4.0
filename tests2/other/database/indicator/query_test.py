...
def test_from_ids_with_table_items(
    ...
    assert Indicator.query.from_ids(ids=set()) == []
    assert Indicator.query.from_ids(ids={NON_EXISTENT_ID}) == []
    assert Indicator.query.from_ids(ids={item_1.id}) == [item_1]
    assert Indicator.query.from_ids(ids={item_2.id}) == [item_2]
    assert Indicator.query.from_ids(ids={item_1.id, item_2.id}) == other_indicators
    assert (
...
def test_from_identifier_with_table_items(
    ...
    assert Indicator.query.from_identifiers(identifiers=set()) == []
    assert Indicator.query.from_identifiers(identifiers={NON_EXISTENT_IDENTIFIER}) == []
    assert Indicator.query.from_identifiers(identifiers={item_1.identifier}) == [item_1]
    assert Indicator.query.from_identifiers(identifiers={item_2.identifier}) == [item_2]
    assert (
