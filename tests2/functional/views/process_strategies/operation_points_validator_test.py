...
@pytest.mark.parametrize(
    ...
    [
        ...
        # all money management strategies have the same dates
        (
            generate_random_long_operation_points(1, "EURUSD", "2024-01-01", 2)
            + generate_random_long_operation_points(2, "EURUSD", "2024-01-04", 2),
            generate_random_short_operation_points(1, "EURUSD", "2024-01-01", 2)
            + generate_random_short_operation_points(2, "EURUSD", "2024-01-04", 2),
        ),
    ],
)
def test_operation_points_validation_raises(
    ...
    with pytest.raises(InvalidOperationPointsError):
        OperationPointsValidator(long_operation_points, short_operation_points).run()
